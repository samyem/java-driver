...
import com.datastax.driver.core.TypeCodec.TextCodec;
...
public class CodecRegistryTest {
    ...
    public void should_retrieve_codec_by_java_and_cql_when_simple_type(){
        CodecRegistry registry = CodecRegistry.builder().withCodecs(TextCodec.instance).build();
        assertThat(registry.codecFor(text(), String.class)).isSameAs(TextCodec.instance);
    }
    ...
    public void should_override_codec(){
        CodecRegistry registry = CodecRegistry.builder().withCodecs(TextCodec.instance, VarcharCodec.instance).build();
        assertThat(registry.codecFor("foo")).isSameAs(VarcharCodec.instance);
        assertThat(registry.codecFor(varchar())).isSameAs(VarcharCodec.instance);
        assertThat(registry.codecFor(text())).isSameAs(TextCodec.instance);
        assertThat(registry.codecFor(text(), String.class)).isSameAs(TextCodec.instance);
    }

    @Test(groups = "unit")
    public void should_override_codec2(){
        TypeCodec<?> codec1 = new FakeCodec<Integer>(cint(), of(Integer.class));
        TypeCodec<?> codec2 = new FakeCodec<Integer>(cint(), of(Integer.class));
        TypeCodec<?> codec3 = new FakeCodec<BigInteger>(cint(), of(BigInteger.class));
        CodecRegistry registry = CodecRegistry.builder().withCodecs(codec1, codec2, codec3).build();
        assertThat(registry.codecFor(cint())).isSameAs(codec3);
        assertThat(registry.codecFor(cint(), of(Integer.class))).isSameAs(codec2);
        assertThat(registry.codecFor(cint(), of(BigInteger.class))).isSameAs(codec3);
    }
    ...
    public void test_interface_vs_implementation(){
        // List<String>
        TypeCodec<?> codec = new ListCodec<String>(TextCodec.instance);
        CodecRegistry registry = CodecRegistry.builder().withCodecs(codec).build();
    ...
    private class ArrayListOfStringsCodec extends TypeCodec.CollectionCodec<String, ArrayList<String>> {
        ...
        public ArrayListOfStringsCodec() {
            super(list(text()), new TypeToken<ArrayList<String>>(){}, TextCodec.instance);
        }
